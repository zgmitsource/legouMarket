    ...
    $.fn.extend({
        slidebox: function (opts) {
            ...
            $(this).each(function (index, item) { //此处循环是因为选择器可能选择了页面上多个轮播
                ...
                $(item).find(".innerwrapper").css({
                    "left": -boxwidth,
                    "height": boxheight
                }); //修改innerwrapper初始位置的坐标
                $(item).find(".innerwrapper>li").css({
                    "width": boxwidth,
                    "height": boxheight,
                    "padding": "0",
                    "display": "block"
                ...
                $(item).find(".btnleft").css({
                    "opacity": "0.1"
                }); //设置按钮的透明度
                $(item).find(".btnright").css({
                    "opacity": "0.1"
                }); //设置按钮的透明度
                ...
                $(item).find(".controls").css({
                    "width": w2 + "px",
                    "position": "absolute",
                    "height": prams.controlsH,
                    "left": left2,
                    "bottom": prams.controltop,
                    "background-color": "transparent"
                });
                $(item).find(".controls>li").text("").css({
                    "width": prams.controlsW,
                    "height": prams.controlsH,
                    "border-radius": prams.radius,
                    "padding": "0",
                    "display": "block",
                    "border": "0"
                });
                ...
                $(item).find(".controls>li").css({
                    "background-color": prams.controlsColor
                });
                //默认对第一个设置为当前按钮的颜色
                $(item).find(".controls>li").eq(0).css({
                    "background-color": prams.controlsCurrentColor
                });
                ...
                function playppt() {
                    ...
                    $(item).find(".innerwrapper").animate({
                        "left": leftx
                    }, function () {
